<template>
  <div class="Servicecontract">
    <div class="Servicecontract_list">
      <el-table :data="tableData" style="width:60%">
        <el-table-column type="index"></el-table-column>
        <el-table-column prop="name" label="姓名"></el-table-column>
        <el-table-column prop="phone" label="新电话号码"></el-table-column>
        <el-table-column prop="time" label="日期"></el-table-column>
        <el-table-column label="操作" width="180px">
          <template slot-scope="scope">
            <el-button size="mini" type="danger" @click="Querylist(scope.$index, scope.row)">查看</el-button>
            <el-button size="mini" type="danger" @click="handlexport(scope.$index, scope.row)">导出</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <div class="Servicecontract_paging">
      <el-pagination
    ...
    <el-dialog title="查看入网协议导出" :visible.sync="dialogVisible">
       <div class="Service">
            <p class="user_name">甲方：{{name}}</p>
            <p>
            ...
            1、个人客户：提供个人有效身份证件原件，委托他人办理的应同时提交本人及代理人有效身份证件原件及授权委托书。根据《中华人民共和国民法通则》要求，10周岁以下未成年人不具备民事行为能力，无法办理入网手续，须由法定代理人出示本人证件原件办理。户口簿仅供10（含）至16岁（含）的未成年人办理入网手续，须同时查验、登记并留存法定代理人身份证件信息。
            ...
            （五）甲方本人未到场，由代理人、经办人办理各类业务的，甲方应承担由此产生的责任与义务；代理人、经办人承担连带责任。
            第二条费用标准和费用交纳
            ...
            （四）若甲方为后付费用户，每月1日至25日为支付上月费用的交费期限。甲方应按时交纳通信费用。甲方未在约定期限内足额交纳通信费用的，乙方每日加收所欠费用的３‰作为违约金，并有权暂停甲方服务；欠费3个月内，甲方交清欠费和违约金后，除甲方明确提出不开通或已销号外，乙方应在24小时内恢复甲方服务。对前述情形，乙方将保留追缴欠费、违约金及向征信机构提供用户欠费信息的权利，也可用通知单、委托第三方等形式追缴欠费。
            ...
            第三条客户权益
            （一）网络服务
            1、乙方在现有技术条件下3G及以上网络覆盖范围内向甲方提供通信服务。
            ...
            （六）若非双方另行约定，乙方提供的宽带接入线路最多限定5个终端上网使用，未经乙方书面同意，甲方不得私自组网连接其他终端设备。
            ...
            （十六）甲方须使用与乙方提供的3G及以上网络技术标准匹配的终端设备，以保证正常使用乙方提供的通信服务，如因甲方终端制式不匹配造成甲方的损失，由甲方自行承担。
            第五条违约责任
            ...
            <p class="user_namex">甲方:(签字或盖章){{name}}</p>
            <p> 乙方:(盖章):中国联合网络通信有限公司成都市分公司</p>                          
            <p class="number">甲方用户号码:{{number}}</p> 
            <p class="currentDate">当下日期:{{date}}</p>
    </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      _this.$httpRequest({
        ...
        success: resp => {
          ...
          if (resp.data.msg=="查询成功") {
           this.tableData=resp.data.date.personmanage;
    ...
    handlexport(index,row){
      location.href = "http://www.lixiangping.cn:10001/personmanage/download?id="+row.id;
    },
